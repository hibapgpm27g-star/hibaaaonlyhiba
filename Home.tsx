...
const awards = [
  { id: "strategy-showdown", rank: "1st", title: "Strategy Showdown", org: "GLIM", year: "2026" },
  { id: "price-marketify", rank: "3rd", title: "Price Marketify", org: "Great Lakes Institute of Management", year: "2026" },
  { id: "employee-of-the-year", rank: "Winner", title: "Employee of the Year", org: "Happenstance", year: "2024" },
  { id: "footwear-design-award", rank: "Bronze", title: "International Footwear Design Award", org: "Global Footwear Awards", year: "2021" },
];
const awardColors = ["bg-pure-sun", "bg-airplane-view", "bg-bubble-gum", "bg-limeade"];
      ...
      <header className="sticky top-4 z-40 mx-4 rounded-full border-2 border-ink bg-card/90 shadow-[4px_4px_0_var(--ink)] backdrop-blur-xl sm:mx-8 lg:mx-auto lg:max-w-[calc(100%-6rem)]" data-testid="portfolio-header">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-3 sm:px-6" data-testid="portfolio-navigation">
          ...
          <nav className={`${mobileMenuOpen ? "absolute left-0 right-0 top-[calc(100%+0.75rem)] flex flex-col rounded-3xl border border-border bg-card p-4 shadow-xl" : "hidden"} gap-1 lg:static lg:flex lg:flex-row lg:items-center lg:gap-1 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none`} aria-label="Main navigation" data-testid="main-navigation-links">
            {[["About", "about"], ["Pillars", "pillars"], ["Projects", "projects"], ["Experience", "experience"], ["Awards", "awards"], ["Contact", "contact"]].map(([label, id]) => (
      ...
      <main id="top" data-testid="portfolio-main">
        <section className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-20 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:gap-20 lg:px-12 lg:pb-28 lg:pt-28" data-testid="hero-section">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.12 }} className="relative z-10 lg:pb-8" data-testid="hero-portrait-panel">
            <div className="relative ml-auto max-w-md" data-testid="hero-portrait-frame">
              ...
              <div className="sticker-float absolute -bottom-8 -left-8 w-52 rounded-2xl border-2 border-ink bg-pop-blue p-5 shadow-[5px_5px_0_var(--ink)]" data-testid="hero-note-card"><Sparkles className="size-5 text-brand" aria-hidden="true" /><p className="mt-3 font-heading text-lg leading-tight text-[#25151f]" data-testid="hero-note-text">Consultant's mind.</p><p className="mt-2 text-xs leading-5 text-[#25151f]/70" data-testid="hero-note-supporting-text">Product manager's hands.</p></div>
            </div>
          </motion.div>
        </section>
        ...
        <section id="about" className="border-y-2 border-ink bg-peony-bundle" data-testid="about-section">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.75fr_1.25fr] lg:gap-24 lg:px-12 lg:py-28">
            <div data-testid="about-intro"><p className="eyebrow" data-testid="about-eyebrow">01 / The plot twist</p><h2 className="mt-5 max-w-sm font-heading text-3xl font-semibold leading-tight tracking-tight text-ink sm:text-4xl" data-testid="about-heading">I find the problem, build the solution, and ship it.</h2></div>
            <div data-testid="about-content"><p className="max-w-3xl text-xl leading-9 text-ink sm:text-2xl" data-testid="about-summary">Product builder with a consultant's mind. Three years of taking ideas from customer insight to market, now sharpened by an MBA.</p><p className="mt-7 max-w-2xl text-base leading-7 text-[#25151f]/70" data-testid="about-detail">I've owned 50+ products end to end: understanding what customers need, turning it into requirements, working with 15+ suppliers and cross-functional teams, and taking it to market. The outcome: ~40% revenue growth, 60% faster time-to-market, 45% more repeat customers and ~15% lower costs. I think like a consultant (structure the problem, let data decide) and deliver like a product manager (prioritise, align teams, ship). At Great Lakes, I'm adding strategy and analytics to that foundation.</p><div className="mt-10 flex flex-wrap gap-3" data-testid="about-tags">{["Gurgaon, India", "Customer-first product thinking", "Structured problem-solving", "Data-backed decisions"].map((tag) => <span key={tag} className="-rotate-1 rounded-full border-2 border-ink bg-background px-4 py-2 text-xs font-semibold text-ink shadow-[2px_2px_0_var(--pure-sun)]" data-testid={`about-tag-${tag.toLowerCase().replaceAll(", ", "-").replaceAll(" ", "-")}`}>{tag}</span>)}</div></div>
          </div>
        </section>
