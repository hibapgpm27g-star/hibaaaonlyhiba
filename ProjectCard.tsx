    ...
    >
      <div className={`relative aspect-[1.2/0.78] overflow-hidden bg-muted ${project.isFeatured ? "lg:aspect-auto lg:min-h-[31rem]" : ""}`} data-testid={`project-image-frame-${project.id}`}>
        <img
          ...
          className="h-full w-full object-cover grayscale-[0.85] contrast-[1.05] transition-[transform,filter] duration-500 group-hover:scale-105 group-hover:grayscale-0"
        />
      ...
      <div className={`space-y-4 p-6 sm:p-7 ${project.isFeatured ? "lg:flex lg:flex-col lg:justify-between lg:p-10" : ""}`} data-testid={`project-card-content-${project.id}`}>
        <div className="flex items-start justify-between gap-4" data-testid={`project-card-heading-${project.id}`}>
          <div>
            ...
            {project.logoUrl ? (
              <img src={project.logoUrl} alt={`${project.brandMark ?? project.title} logo`} className="mt-2 h-7 w-auto object-contain" data-testid={`project-brand-mark-${project.id}`} />
            ) : project.brandMark ? (
              <span className="mt-2 inline-block font-heading text-xs font-semibold tracking-[0.24em] text-tomato-jam" data-testid={`project-brand-mark-${project.id}`}>{project.brandMark}</span>
            ) : null}
